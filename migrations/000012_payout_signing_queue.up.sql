...
ALTER TABLE payouts
    ...
    ADD COLUMN execution_lease_epoch BIGINT NOT NULL DEFAULT 0 CHECK (execution_lease_epoch >= 0),
    ADD COLUMN signing_attempts INTEGER NOT NULL DEFAULT 0 CHECK (signing_attempts >= 0),
    ADD COLUMN transaction_id CHAR(64),
    ADD COLUMN signed_transaction BYTEA,
    ADD COLUMN last_execution_error TEXT,
...
CREATE INDEX payouts_signing_claim_idx
    ON payouts (updated_at, id)
    WHERE status = 'approved';

CREATE UNIQUE INDEX payouts_transaction_id_unique_idx
