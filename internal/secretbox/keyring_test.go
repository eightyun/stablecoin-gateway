...
func TestKeyringEncryptsAndRotatesSecrets(t *testing.T) {
	...
	secret := []byte("a-secure-application-secret")
	encrypted, err := keyring.Encrypt("resource-id", secret)
	if err != nil || encrypted.Version != "v2" || bytes.Contains(encrypted.Ciphertext, secret) {
		t.Fatalf("Encrypt() = %+v, %v", encrypted, err)
	}
	decrypted, err := keyring.Decrypt("resource-id", encrypted)
	if err != nil || !bytes.Equal(decrypted, secret) {
		t.Fatalf("Decrypt() = %q, %v", decrypted, err)
	}
	if _, err := keyring.Decrypt("other-resource", encrypted); !errors.Is(err, ErrInvalidSecret) {
		t.Fatalf("篡改 AAD 后 Decrypt() error = %v", err)
	}
}
